...
#       layoutlmv3_model = torch.load(model_path).to(torch_device)
#     return layoutlmv3_model

def load_model(model_path):
    # torch_device = 'cuda' if torch.cuda.is_available() else 'cpu'
    model = AutoModelForTokenClassification.from_pretrained(model_path)#.to(torch_device)
